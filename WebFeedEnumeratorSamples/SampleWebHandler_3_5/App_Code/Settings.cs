...
public static class Settings
{
    ...
    public static bool UseRoles
    {
        get
        {
            string rtn = System.Configuration.ConfigurationManager.AppSettings["UseRoles"];
            if (string.IsNullOrEmpty(rtn))
                return false;
            rtn = rtn.Trim();
            return rtn.Equals("true", StringComparison.OrdinalIgnoreCase) || rtn.Equals("1", StringComparison.Ordinal);
        }
    }
    ...
    {
        JavaScriptSerializer rtn = new JavaScriptSerializer();
        rtn.MaxJsonLength = int.MaxValue;
        rtn.RegisterConverters(new JavaScriptConverter[] {
                    new JavascriptSerializerDateTimeConverter()
                });
        return rtn;
    }
}